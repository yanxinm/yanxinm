...
### Python 3.10 incompatibility with `cli-hub`

`cli-anything-hub` v0.3.0 uses f-string backslash syntax (`f"\\n"` inside `{}`) which requires **Python ≥ 3.12**. On Ubuntu 22.04 (Python 3.10), `cli-hub install` crashes with:

```
SyntaxError: f-string expression part cannot include a backslash
```

**Workaround:** Install skills manually instead of via `cli-hub`:

```bash
# Pull SKILL.md directly
mkdir -p ~/.hermes/skills/<skill-name>
curl -sL "https://raw.githubusercontent.com/HKUDS/CLI-Anything/main/<skill-dir>/SKILL.md" \
  -o ~/.hermes/skills/<skill-name>/SKILL.md
```

Hermes auto-discovers skills in `~/.hermes/skills/` — no `cli-hub` needed.

### GitHub blocked from China

`raw.githubusercontent.com` is blocked by GFW. Mirrors like `raw.githubusercontent.moeyy.xyz` work intermittently. HTTPS clone via `github.moeyy.xyz` also times out for large repos.

**Workaround:** If the full repo (including `cli-anything-plugin/HARNESS.md`) can't be cloned, use this skill's condensed 7-phase rules below as fallback. The condensed rules are self-contained and work without external files.
